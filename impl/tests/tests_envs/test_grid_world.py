...
class TestGridWorldEnv:
    def test_init(self):
        env = GridWorldEnv(
            ...
            target_loc=(1, 1),
            size=5,
            forbidden_area_cfg=ForbiddenAreaCfg(locs=[(0, 0)]),
        )
        ...
        env = GridWorldEnv(
            ...
            target_loc=None,
            size=(2, 2),
            forbidden_area_cfg=ForbiddenAreaCfg(locs=[(0, 0), (0, 1), (1, 0)]),
        )
        assert is_same_array(env._target_loc, (1, 1), True)
        assert env.size == (2, 2)
        ...
        with pytest.raises(ValueError):
            env = GridWorldEnv(
                ...
                target_loc=(1, 1),
                size=2,
                forbidden_area_cfg=ForbiddenAreaCfg(num=4),
            )
    ...
    def test_reset(self):
        ...
        obs, _ = env.reset(options={"agent_loc": (1, 1)})
